// Fixed Map Modal Functionality
class MapModal {
    constructor() {
        console.log('MapModal constructor called');
    ...
    applyTranslations() {
        ...
        console.log('MapModal initialized and translations applied');
    }
    ...
    getSimplifiedMapData() {
        return [
            ...
            { code: 'GHA', name: 'Ghana', path: 'M 210 290 L 235 285 L 240 305 L 210 310 Z' },
    ...
    getCountryRegion(countryCode) {
        for (const [region, data] of Object.entries(this.countries)) {
            if (region === 'middleEast') {
                ...
                // Check all subregions (which are arrays)
                for (const subregion of Object.values(data)) {
                    if (Array.isArray(subregion) && subregion.includes(countryCode)) {
                        return region;
                    }
                }
            }
    ...
    setupEventListeners() {
        ...
        setTimeout(() => {
            ...
            console.log('Looking for open-map-btn button:', this.openMapBtn);
            ...
            if (this.openMapBtn) {
                ...
                // Also try mousedown/mouseup as backup
                this.openMapBtn.addEventListener('mousedown', (e) => {
                    console.log('MOUSE DOWN on button');
                });
                
                this.openMapBtn.addEventListener('mouseup', (e) => {
                    console.log('MOUSE UP on button');
                    // Try to trigger modal on mouseup as fallback
                    if (e.button === 0) { // Left click
                        console.log('Left mouse up, triggering modal...');
                        this.showModal();
                    }
                });
                ...
                console.log('Button element:', this.openMapBtn);
                console.log('Button offset:', this.openMapBtn.getBoundingClientRect());
                
                // Check if button has pointer-events
                const computedStyle = window.getComputedStyle(this.openMapBtn);
                console.log('Button pointer-events:', computedStyle.pointerEvents);
                console.log('Button cursor:', computedStyle.cursor);
                
                // Add a global click listener as last resort
                document.addEventListener('click', (e) => {
                    if (e.target === this.openMapBtn || e.target.closest('#open-map-btn')) {
                        console.log('Global click listener caught the button click!');
                        e.preventDefault();
                        e.stopPropagation();
                        this.showModal();
                    }
                ...
                console.error('Map button not found! Available buttons:', 
                    document.querySelectorAll('button[id*="map"], button[id*="open"]').length);
                ...
                allButtons.forEach((btn, index) => {
                    if (btn.textContent.includes('Pays') || btn.textContent.includes('Découvrir')) {
                        console.log(`Button ${index}:`, btn.textContent, btn.id);
                    }
                });
            }
            ...
            if (this.svg) {
                ...
                console.log('Found country paths:', countryPaths.length);
                countryPaths.forEach((path, index) => {
                    path.addEventListener('mouseenter', (e) => {
                        console.log(`Hovering country ${index}`);
                        this.showTooltip(e);
                    });
                    path.addEventListener('mouseleave', () => {
                        console.log('Left country');
                        this.hideTooltip();
                    });
                    path.addEventListener('mousemove', (e) => this.updateTooltipPosition(e));
                });
            } else {
                console.log('SVG not found yet');
            }
            
            // Check if modal content is properly structured
            const modalContent = this.modal.querySelector('.map-modal-content');
            if (modalContent) {
    ...
    // Show modal
    showModal() {
        console.log('showModal called, modal:', this.modal);
        if (this.modal) {
            console.log('Modal found, showing...');
            
            // Remove any potential conflicting styles from parent elements
            const parent = this.modal.parentElement;
            if (parent) {
                parent.style.position = 'relative';
                parent.style.zIndex = 'auto';
            }
            
            // Add debug styles to make modal visible with highest z-index
            ...
            console.log('Modal styles applied:', this.modal.style.cssText);
            ...
            setTimeout(() => {
                ...
                if (rect.width === 0 || rect.height === 0) {
                    console.error('Modal has no dimensions!');
                } else {
                    console.log('Modal has proper dimensions');
                }
            }, 100);
    ...
    hideModal() {
        ...
        if (this.modal) {
            ...
            console.log('Modal not found for hiding');
        }
    }
    ...
    showTooltip(event) {
        ...
        console.log('Showing tooltip for:', countryName, countryCode, region);

        if (!this.tooltip || !countryName) {
            console.log('Tooltip or country name not found');
            return;
        }
        ...
        if (titleElement) {
            ...
            console.log('Tooltip title element not found');
        }
        ...
        if (regionElement) {
            ...
            console.log('Tooltip region element not found');
        }

        console.log('Tooltip content set, showing tooltip');
    ...
    updateTooltipPosition(event) {
        if (!this.tooltip) {
            console.log('Tooltip not found for positioning');
            return;
        }
        ...
        console.log('Tooltip position updated to:', x, y);
    }
...
function initializeMapModal() {
    console.log('Initializing MapModal...');
...
setTimeout(() => {
    if (!mapModalInstance) {
        console.log('Initializing MapModal as backup...');
        initializeMapModal();
    }
}, 1000);
